...
- Components may branch on progress identifiers such as `type` and `setupStep`,
  but visible text must use user-facing copy from `progress.message` or an
  explicit label map. Do not render raw ids such as `runtime_setup` or
...
- Onboarding is visible only when Docker is unavailable and there are no images
  ...
  runtime setup. During setup, prefer the shell-provided progress message and
  omit raw setup-step detail unless it has been mapped to product copy.
